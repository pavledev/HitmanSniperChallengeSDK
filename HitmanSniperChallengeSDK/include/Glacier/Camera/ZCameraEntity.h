...
class HitmanSniperChallengeSDK_API ICameraEntity : public IComponentInterface
{
public:
...
class HitmanSniperChallengeSDK_API alignas(16) ZCameraEntity : public ZRenderableEntity, public ICameraEntity
{
public:
...
private:
	PAD(0x4);
	float m_fFovYDeg; // 0x88
	PAD(0xC);
	float m_fNearZ; // 0x98
	float m_fFarZ; // 0x9C
	PAD(0x1C);
	TEntityRef<IRenderPostfilterControllerEntity> m_rPostfilter; //0xBC
	PAD(0x10);
	float m_fFovY; // 0xD4
};
