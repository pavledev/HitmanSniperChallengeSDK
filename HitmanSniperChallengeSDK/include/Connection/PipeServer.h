...
public:
	...
	~PipeServer();
	HitmanSniperChallengeSDK_API static PipeServer& GetInstance();
	void Start();
	void Stop();
	void Update();
	HitmanSniperChallengeSDK_API void SendMessage(const std::string& type, const std::string& content);
	void SetMessageCallback(MessageCallback messageCallback);
	void SetConnectionCallback(ConnectionCallback connectionCallback);
	HitmanSniperChallengeSDK_API bool IsConnectedWithEditor() const;
