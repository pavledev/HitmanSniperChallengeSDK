...
public:
	...
	~Logger();
	HitmanSniperChallengeSDK_API static Logger& GetInstance();
	HitmanSniperChallengeSDK_API static unsigned int LevelToNumber(const Level level);
	HitmanSniperChallengeSDK_API static const char* LevelToString(const Level level);
	std::vector<Message>& GetMessages();
	void ClearMessage(const unsigned int index);
	void ClearAllMessages();
	HitmanSniperChallengeSDK_API static std::string GetCurrentTime();
