...
ZVariant ZEntityRef::GetProperty(const unsigned int propertyID)
{
	...
	if (propertyData)
	{
		...
		const unsigned int propertyAddress = reinterpret_cast<uintptr_t>(m_pEntityTypePtrPtr) + propertyData->m_pInfo.m_nPropertyOffset + propertyData->m_pInfo.m_nExtraData;
