...
bool DirectXRenderer::Setup()
{
	...
	originalWndProc = reinterpret_cast<WNDPROC>(SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc)));

	if (originalWndProc)
	{
		Logger::GetInstance().Log(Logger::Level::Error, "Successfully set WndProc.");
	}
	else
	{
		Logger::GetInstance().Log(Logger::Level::Error, "Failed to set WndProc.");
	}
...
void DirectXRenderer::OnPresent()
{
	if (!Setup())
	{
	...
	ID3D11DeviceContext* immediateContext = RenderManager->GetRenderDevice()->GetImmediateContext();
