...
#include "Glacier/Input/ZInputDeviceManagerWindows.h"
...
long ImGuiRenderer::OnMainWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (scanCode == 0x29 && (uMsg == WM_KEYDOWN || uMsg == WM_SYSKEYDOWN))
    {
        ...
        if (!ScaleformManager->IsInMainMenu() && !HUDManager->IsPauseMenuActive())
        {
            if (imguiHasFocus)
            {
                SetCursor(ZApplicationEngineWin32::GetInstance()->GetDefaultCursor());
            }
            else
            {
                SetCursor(nullptr);
            ...
            ZApplicationEngineWin32::GetInstance()->SetShowingCursor(imguiHasFocus);
        }
        else if (ScaleformManager->IsInMainMenu() || HUDManager->IsPauseMenuActive())
        {
            ZInputDevice* mouseInputDevice = static_cast<ZInputDevice*>(InputDeviceManager->GetNthDevicePtr(IInputDevice::EDeviceType::eMOUSE_TYPE, 0));
            ZInputDevice* keyboardInputDevice = static_cast<ZInputDevice*>(InputDeviceManager->GetNthDevicePtr(IInputDevice::EDeviceType::eKEYBOARD_TYPE, 0));

            // Block mouse and keyboard input in main menu and pause menu
            mouseInputDevice->m_active = !imguiHasFocus;
            keyboardInputDevice->m_active = !imguiHasFocus;
        }
    }
