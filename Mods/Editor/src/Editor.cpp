...
#include <Glacier/Morpheme/ZAnimationDatabase.h>
#include <Glacier/Morpheme/ZAnimationResource.h>
#include <Glacier/Morpheme/ZNetworkResource.h>
#include <Glacier/Morpheme/NodeDefAnim.h>
...
void Editor::RenderEntityTree(const bool hasFocus)
{
    ...
    const bool isWindowVisible = ImGui::Begin(ICON_MD_VIEW_LIST " Entity Tree", nullptr, ImGuiWindowFlags_HorizontalScrollbar);
    ...
    if (!rootNode->entityRef.GetEntityTypePtrPtr())
    {
        ...
        rootNode->entityRef = sceneEntityRef;
    }
...
void Editor::RenderEntityProperties(const bool hasFocus)
{
    ...
    const bool isWindowVisible = ImGui::Begin(ICON_MD_BUILD " Entity Properties", nullptr, ImGuiWindowFlags_HorizontalScrollbar);
...
void Editor::RenderBlueprintNodesAndPins(const bool hasFocus)
{
    ...
    const bool isWindowVisible = ImGui::Begin(ICON_MD_BUILD " Blueprints Graph View", nullptr, ImGuiWindowFlags_HorizontalScrollbar);
...
void Editor::AddChildren(std::shared_ptr<EntityTreeNode> entityTreeNode, ZEntityRef rootEntity, ZTemplateEntityBlueprintFactory* templateEntityBlueprintFactory, const int parentIndex, STemplateEntityBlueprint* templateEntityBlueprint)
{
    ...
    {
        DeserializeTemplateEntityBlueprint(tbluRuntimeResourceID);
    }
...
void Editor::OnEntityTransformChange(std::shared_ptr<EntityTreeNode> entityTreeNode, const SMatrix& transform, bool relative)
{
    ...
    if (staticPhysicsAspect && staticPhysicsAspect->m_pPhysicsObject)
    {
        staticPhysicsAspect->m_pPhysicsObject->SetTransform(spatialEntity->GetObjectToWorldMatrix());
    }
}
...
void Editor::LoadHeaderLibraries()
{
    std::string runtimeFolderPath = std::format("{}\\Runtime", std::filesystem::current_path().string());

    for (auto& p : std::filesystem::recursive_directory_iterator(runtimeFolderPath))
    {
        ...
        HeaderLibrary& headerLibrary = headerLibraries.emplace_back();

        BinaryReader binaryReader = BinaryReader(p.path().string());

        binaryReader.Seek(4, SeekOrigin::Current);

        unsigned int referencesChunkSize = binaryReader.Read<unsigned int>();

        binaryReader.Seek(referencesChunkSize + 16, SeekOrigin::Current);

        const unsigned int dataSize = binaryReader.GetSize() - binaryReader.GetPosition();
        void* data = binaryReader.Read<void>(dataSize);

        headerLibrary.headerLibrary = static_cast<SHeaderLibrary*>(binaryDeserializer.Deserialize(data, dataSize));

        std::filesystem::path folder = p.path().parent_path();

        for (auto& file : std::filesystem::directory_iterator(folder))
        {
            if (file.path().extension() == ".pc_idmap")
            {
                LoadResourcesInfo(file.path().string(), headerLibrary);
...
void Editor::LoadResourcesInfo(const std::string& idMapPath, HeaderLibrary& headerLibrary)
{
    bool isGlobalHeaderLibrary = idMapPath.contains("globalresources.ini");
    BinaryReader binaryReader = BinaryReader(idMapPath);

    binaryReader.Seek(24, SeekOrigin::Begin);

    std::string line;
    std::string previousLibraryID;
    unsigned int chunkIndex = 0;
    unsigned int resourceIndex = 0;
    unsigned int resourceDataOffset = 24;

    while (binaryReader.GetPosition() < binaryReader.GetSize())
    {
        const char c = binaryReader.Read<char>();

        if (c == '\n')
        {
            unsigned long long runtimeResourceID = std::strtoull(line.substr(1, 16).c_str(), nullptr, 16);
            SResourceHeaderHeader* resourceHeaderHeader = nullptr;

            if (line.starts_with("|00"))
            {
                resourceHeaderHeader = reinterpret_cast<SResourceHeaderHeader*>(headerLibrary.headerLibrary->chunks[chunkIndex].resourceHeaders[resourceIndex].GetStart());

                headerLibrary.runtimeResourceIDs.insert(std::make_pair(runtimeResourceID, std::make_tuple(chunkIndex, resourceDataOffset, resourceHeaderHeader->m_nDataSize)));
            }
            else
            {
                std::string currentLibraryID = line.substr(9, 8);

                if (!isGlobalHeaderLibrary && previousLibraryID.empty())
                {
                    previousLibraryID = currentLibraryID;
                }

                if (currentLibraryID != previousLibraryID)
                {
                    previousLibraryID = currentLibraryID;

                    ++chunkIndex;
                    resourceIndex = 0;
                    resourceDataOffset = 24;
                }
            ...
            resourceDataOffset += resourceHeaderHeader->m_nDataSize;
...
void Editor::DeserializeTemplateEntityBlueprint(unsigned long long runtimeResourceID)
{
    for (auto it = headerLibraries.begin(); it != headerLibraries.end(); ++it)
    {
        auto it2 = it->runtimeResourceIDs.find(runtimeResourceID);

        if (it2 != it->runtimeResourceIDs.end())
        {
            const unsigned int chunkIndex = std::get<0>(it2->second);
            const unsigned int resourceDataOffset = std::get<1>(it2->second);
            const unsigned int resourceSize = std::get<2>(it2->second);

            std::string filePath = ConvertResourceIDToFilePath(it->headerLibrary->chunks[chunkIndex].sInstallID.ToCString());
            filePath = std::format("{}{}", std::filesystem::current_path().string(), filePath.substr(2));
            BinaryReader binaryReader = BinaryReader(filePath);

            binaryReader.Seek(resourceDataOffset);

            void* resourceData = binaryReader.Read<void>(resourceSize);
            ZBinaryDeserializer binaryDeserializer;
            STemplateEntityBlueprint* templateEntityBlueprint = static_cast<STemplateEntityBlueprint*>(binaryDeserializer.Deserialize(resourceData, resourceSize));

            templateEntityBlueprints[it2->first] = templateEntityBlueprint;
        }
    }
}

std::string Editor::ConvertResourceIDToFilePath(std::string resourceID)
{
    ...
    Hash::ConvertMD5ToCString(Hash::MD5(resourceID), fileName, sizeof(fileName));
