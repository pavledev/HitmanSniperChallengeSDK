...
private:
    ...
    struct HeaderLibrary
    {
        ...
        std::unordered_map<unsigned long long, std::tuple<unsigned int, unsigned int, unsigned int>> runtimeResourceIDs;
    };
    ...
    void LoadHeaderLibraries();
    void LoadResourcesInfo(const std::string& idMapPath, HeaderLibrary& headerLibrary);
    void DeserializeTemplateEntityBlueprint(unsigned long long runtimeResourceID);
    std::string ConvertResourceIDToFilePath(std::string resourceID);
    ...
    std::vector<HeaderLibrary> headerLibraries;
};
